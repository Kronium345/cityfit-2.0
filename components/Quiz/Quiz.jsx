...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://192.168.1.216:5000/quiz/quiz'); // Use your backend's /quiz route
...
const handleSubmit = async () => {
  ...
  if (!isError) {
    try {
      ...
      const response = await fetch('http://192.168.1.216:5000/quiz/predict', {
        method: 'POST',
        headers: {
