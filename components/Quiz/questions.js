export const qtn = [
    {
      name: "s3",
      text: "Are you happy about your academic condition?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s4",
      text: "Do you have family history of mental illness?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s5",
      text: "Are you addicted to drugs?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s6",
      text: "Do you have sadness from death or loss?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s7",
      text: "Do you have financial problems?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s8",
      text: "Violence in family?",
      options: [
        {
          text: "Most of the time",
          value: 0,
        },
        {
          text: "Never",
          value: 1,
        },
        {
          text: "Often",
          value: 2,
        },
        {
          text: "Rarely",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s9",
      text: "Have you ever been bullied?",
      options: [
        {
          text: "No",
          value: 0,
        },
        {
          text: "Yes",
          value: 1,
        },
      ],
      selected: null,
    },
    {
      name: "s10",
      text: "Does bullying still affect you?",
      options: [
        {
          text: "Most of the time",
          value: 0,
        },
        {
          text: "Never",
          value: 1,
        },
        {
          text: "Often",
          value: 2,
        },
        {
          text: "Rarely",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s11",
      text: "How many hours do you spend on social media?",
      options: [
        {
          text: "1 to 3 hours",
          value: 0,
        },
        {
          text: "3 to 6 hours",
          value: 1,
        },
        {
          text: "Greater than 6 hours",
          value: 2,
        },
        {
          text: "Less than 1 hour",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s12",
      text: "I get angry fast but calm down fast too",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s13",
      text: "When frustrated, I let my irritation show",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s14",
      text: "I sometimes feel like a powder keg ready to explode",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s15",
      text: "I get angry easily.",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s16",
      text: "Some of my friends think I'm a hothead",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s17",
      text: "Sometimes I get angry for no reason",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s18",
      text: "I have trouble controlling my temper",
      options: [
        {
          text: 'Not like me at all',
          value: 0,
        },
        {
          text: 'Not like me',
          value: 1,
        },
        {
          text: "Neutral",
          value: 2,
        },
        {
          text: "Like me",
          value: 3,
        },
        {
          text: "Very like me",
          value: 4,
        },
      ],
      selected: null,
    },
    {
      name: "s19",
      text: "Over the last two weeks, how frequently have you experienced trouble relaxing?",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s20",
      text: "Feeling nervous, anxious, or on edge",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s21",
      text: "Not being able to stop or control worrying",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s22",
      text: "Worrying too much about different things",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s23",
      text: "Being so restless that it's hard to still",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s24",
      text: "Becoming easily annoyed or irritable",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
    {
      name: "s25",
      text: "Feeling afraid as if something awful might happen",
      options: [
        {
          text: "Not at all",
          value: 0,
        },
        {
          text: "Several days",
          value: 1,
        },
        {
          text: "Over half of the days",
          value: 2,
        },
        {
          text: "Nearly everyday",
          value: 3,
        },
      ],
      selected: null,
    },
  ];
  
  
  export const categories = [
    {
      label: 0,
      category: 'Neither anger nor anxiety',
      description: "According to our model's evaluation, it indicates that you are not exhibiting symptoms of either anger or anxiety.",
      suggestion: 'exercise'
    },
    {
      label: 1,
      category: 'Anger Only',
      description: "According to our model's analysis, it predicts that you are currently experiencing symptoms related to anger.",
      suggestion: 'all'
    },
    {
      label: 2,
      category: 'Anxiety Only',
      description: "Based on the data processed by our model, it indicates that you are likely facing symptoms associated with anxiety.",
      suggestion: 'all'
    },
    {
      label: 3,
      category: 'Both anger and anxiety',
      description: 'According to the information provided, our model suggests that you are experiencing symptoms related to both anger and anxiety.',
      suggestion: 'all'
    },
  ]